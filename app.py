...
def main() -> None:
    ...
    with col_left:
        target_col = st.selectbox(
            ...
            options=data_handler.get_column_options(df),
    ...
    if st.button("Train Model", type="primary"):
        with st.spinner("Preprocessing and training..."):
            ...
            X_train, X_test, y_train, y_test = data_handler.split(df_clean, target_col)
            y_train_enc, label_enc = preprocessing.encode_target(y_train)
            y_test_enc, _ = preprocessing.encode_target(y_test)
